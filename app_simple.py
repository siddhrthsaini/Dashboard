...
with st.sidebar:
    ...
    data_source = st.radio(
        "Choose data source:",
        ["📁 Local CSV Files", "☁️ Google Sheets (Coming Soon)"],
        index=0
    )
    ...
    if data_source == "📁 Local CSV Files":
        ...
        **Google Sheets Integration Coming Soon!**
        ...
        To enable live Excel updates:
        1. Follow the `google_sheets_setup.md` guide
        2. Set up Google Sheets API credentials
        3. Download `google-credentials.json`
        4. Restart the app
        
        For now, use the CSV mode to test the dashboard.
        """)
...
if df2 is not None:
    ...
    if data_source == "☁️ Google Sheets (Coming Soon)":
